from flask import Flask, render_template, render_template_string, request, redirect, url_for, jsonify, send_from_directory, flash
import os
import requests
...
def dashboard():
    ...
    for node in nodes:
        ...
        model = meta.get("model", node.get("model", ""))
        user = meta.get("user", "")
        password = meta.get("password", "")
        last = node.get("last", None)
        status = node.get("status", "never")
        ...
        # Look for latest backup config
        folder = os.path.join(BACKUP_DIR, name)
        latest_file = None
        for root, _, files in os.walk(folder):
            txts = sorted([f for f in files if f.endswith(".txt") and f.startswith(f"{name}_")], reverse=True)
            if txts:
...
def delete_device(device_name):
    ...
    with open(db_path, "w") as f:
        for line in lines:
            if not line.strip().startswith(device_name + ":"):
                f.write(line)
